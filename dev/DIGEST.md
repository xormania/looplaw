...
Generated from dev/*.cue, so it cannot fall out of step with the files
...
stake. The schema the binary enforces on input is schema/, and it is
not summarised here.
...
- **human** (BANNED) → name the role and its cardinality: 'the accountable authority — one per deployment, singular, non-delegable'
- **individual** (BANNED) → 'one per deployment' for cardinality; 'the party' for a submitter
- **people** (BANNED) → 'parties', or name the projects or deployments actually meant
- **permit** (BANNED) → 'recorded under standing grant <id>'
- **person** (BANNED) → 'the accountable authority', or 'the party' where any submitter is meant
