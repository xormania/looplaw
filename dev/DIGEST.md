...
- **aa** — all law-making acts (ratify, amend, withdraw, defer, grant); accountability assumed through these recorded acts
...
- **amend** (aa) — ratified law is replaced by a new version; the predecessor is archived, never deleted
- **defer** (aa) — a gap or clause is parked with destination, authority, and trigger
- **grant** (aa) — a standing grant licenses a class of automatic admissions (e.g. guest-mode working sets)
- **ratify** (aa) — a draft becomes law (goal contract, lexicon entry, registry change, Tier 0 invariant, standing grant)
- **record** (recording) — a submission becomes a record (claim, receipt, admission, version) after passing the gates
- **withdraw** (aa) — a law clause or goal is retired without replacement
...
- **approve** (BANNED) → 'ratified by the aa' where law standing is meant; otherwise the non-authoritative verbs (propose, submit, declare)
