// Diff fixture: the library view — derived from the goal fixture with
// three deltas: C-RETURN-1 absent, C-LEND-1 P-1 text changed, C-AUDIT-1
// added (view-only).
...
contracts: {
	"C-LEND-1": {
		...
		acts: ["lend"]
		preconditions: {
			"P-1": {text: "The borrower's membership is in good standing and dues-current, verifiable from the member records at submission."}
	...
	"C-AUDIT-1": {
		...
		acts: ["audit-shelf"]
		preconditions: {
			"P-1": {text: "The audit request names a shelf, verifiable from the catalog records."}
		}
		guarantees: {
			"G-1": {text: "A shelf-audit finding exists for the named shelf.", records: "the audit finding record"}
		}
		invariants_local: {}
		cites: ["L-1"]
		blame: [
			{violation_class: "auditing a phantom shelf", at_fault: "librarian", evidence: "the catalog records at audit time"},
		]
		status: "ratified"
	}
}
