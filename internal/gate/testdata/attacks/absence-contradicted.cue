// ATTACK — a set that declares its judgment register absent and holds
// one.
//
// Both fields are separately well-formed, so the shape gate admits them;
// only reading them together shows the set disagreeing with itself. It
// passed every gate and would have become law that cannot be read for
// either answer. Found by security audit of the master tree.
