// ATTACK — a default inside an embedding.
//
// The same bypass by the other declaration form that carries a value:
// an embedding merges its fields into the enclosing struct, so the
// contract's status is a default, and the walk that examined fields
// never reached it. Found with its sibling attack.
