...
//
// Round 1 (gate): a blame clause attaching to nobody validated green — the reference grammar now refuses the empty string by shape.
...
contracts: {
	...
	"C-RETURN-1": {
		...
		cites: ["L-1"]
		blame: [
			{violation_class: "late return", at_fault: "", evidence: "the loan record's due date against the return record's date"},
		]
		status: "ratified"
	}
}
