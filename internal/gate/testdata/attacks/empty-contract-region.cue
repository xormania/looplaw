// ATTACK — a contract region stating obligations with nothing in them.
//
// The contract is named "", its precondition says nothing, its guarantee
// records nothing, and its blame clause names neither a violation nor
// the evidence it would be adjudicated from. Every field the region
// exists for is present and empty, so the set reads as law and asserts
// none of it — and it passed the gates. Found by security audit of the
// master tree.
...
contracts: {
	"C-LEND-1": {
		...
		cites: ["L-1"]
		blame: [
			{violation_class: "", at_fault: "librarian", evidence: ""},
